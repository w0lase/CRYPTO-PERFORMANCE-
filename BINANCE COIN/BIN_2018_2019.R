## Importing the binancecoin data from 2018 to 2019 ##
...
bin_2018_2019 <- as.data.frame(bin_data_split[1],)

## Generating a vector for the binancecoin prices ##

bin_2018_2019_prices <- bin_2018_2019$X2018.2019.price

## Calculating the log Returns ##

log_ret_bin_2018_2019 <- diff(log(bin_2018_2019_prices))

## Creating a vector to store the negative log returns using threshold (-0.02) ##

neg_ret_bin_2018_2019 <- rep (0, length (log_ret_bin_2018_2019))

## Assigning 1 if negative log returns are less than threshold (-0.02) and 0 otherwise ##

for (i in 1: length(log_ret_bin_2018_2019)) {
  if (log_ret_bin_2018_2019[i] < -0.02) {
    neg_ret_bin_2018_2019[i] <- 1
  }
}

## Creating a vector to store the positive log returns using threshold (0.02) ##

pos_ret_bin_2018_2019 <- rep (0, length (log_ret_bin_2018_2019))

## Assigning 1 if negative log returns are greater than threshold (0.02) and 0 otherwise ##
for (i in 1:length (log_ret_bin_2018_2019)) {
  if (log_ret_bin_2018_2019[i] > 0.02) {
    pos_ret_bin_2018_2019[i] <- 1
  }
}
...
week_summations_bin_2018_2019 <-calc_week_sum_bin(neg_ret_bin_2018_2019)

## Creating a data frame with negative quarters and their corresponding values ##
neg_quart_table_bin_2018_2019 <- data.frame(
  Weekly_bin = 1:104, # Weekly numbers
  Value = week_summations_bin_2018_2019 # Corresponding values
)
# Printing the resulting table

print (neg_quart_table_bin_2018_2019)
...
# Calling the function with 'pos_ret_bin' to get the weekly summations

week_summations_1_bin_2018_2019 <- calc_week_sum_1_bin (pos_ret_bin_2018_2019)

## Creating a data frame with positive weeks and their corresponding values ##

pos_week_table_bin_2018_2019 <- data.frame(
  Week=1:104, #week numbers
  Value = week_summations_1_bin_2018_2019 # Corresponding values
)
# Print the resulting table

print (pos_week_table_bin_2018_2019)

## creating a cross tabulation table ##

cross_tabulation_bin_2018_2019 <-table (week_summations_bin_2018_2019,week_summations_1_bin_2018_2019)


##Adding extra rows and columns

extra_row_bin_2018_2019 <- matrix(rep(0,6),1,6)
rownames(extra_row_bin_2018_2019) <- c("7")
cross_tabulation_bin_2018_2019 <- rbind(cross_tabulation_bin_2018_2019,extra_row_bin_2018_2019)


extra_col_bin_2018_2019 <- matrix(rep(0,16),8,2)
colnames(extra_col_bin_2018_2019) <- c("6","7")
cross_tabulation_bin_2018_2019 <- cbind(cross_tabulation_bin_2018_2019,extra_col_bin_2018_2019)

#Adding row and column margins to the cross-tabulation table

cross_tab_with_totals_bin_2018_2019 <- addmargins (cross_tabulation_bin_2018_2019)

# Printing the table with subtotals and grand totals

print (cross_tab_with_totals_bin_2018_2019)

# Generating row(negative) and column(positive) totals

combined_data_bin_2018_2019 <- rbind(cross_tab_with_totals_bin_2018_2019[,9][-9], cross_tab_with_totals_bin_2018_2019[9,][-9])

combined_data_rf_bin_2018_2019 <- combined_data_bin_2018_2019/104
rownames(combined_data_rf_bin_2018_2019) <- c("Negative","Positive")

list(combined_data_rf_bin_2018_2019[1,],combined_data_rf_bin_2018_2019[2,])

# Create the bar plot with bars side by side of number of change in weekly returns

barplot(combined_data_bin_2018_2019, beside = TRUE, col = c("blue4", "lightblue"),
        names.arg = c(0:7),
        xlab = "Number of change in binancecoin weekly returns",
        ylab = "Frequencies of number of binancecoin weekly returns from 2018 to 2019")
...
neg_pmf_2018_2019 <- combined_data_bin_2018_2019[1,]/sum(combined_data_bin_2018_2019[1,])

pos_pmf_2018_2019 <- combined_data_bin_2018_2019[2,]/sum(combined_data_bin_2018_2019[2,])
...
chunks <- split_list(log_ret_bin_2018_2019, 7)
...
log_ret_bin_2018_2019 <- c(NA,log_ret_bin_2018_2019)
bin_2018_2019 <- cbind(bin_2018_2019,log_ret_bin_2018_2019)


ggplot(bin_2018_2019,aes(x =X2018.2019.date, y = log_ret_bin_2018_2019)) +
