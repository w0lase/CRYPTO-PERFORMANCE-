## Importing the binancecoin data from 2022 to 2023 ##
...
bin_2022_2023 <- as.data.frame(bin_data_split[3],)

## Generating a vector for the binancecoin prices ##

bin_2022_2023_prices <- bin_2022_2023$X2022.2023.price

## Calculating the log Returns ##

log_ret_bin_2022_2023 <- diff(log(bin_2022_2023_prices))

## Creating a vector to store the negative log returns using threshold (-0.02) ##

neg_ret_bin_2022_2023 <- rep (0, length (log_ret_bin_2022_2023))

## Assigning 1 if negative log returns are less than threshold (-0.02) and 0 otherwise ##

for (i in 1: length(log_ret_bin_2022_2023)) {
  if (log_ret_bin_2022_2023[i] < -0.02) {
    neg_ret_bin_2022_2023[i] <- 1
  }
}

## Creating a vector to store the positive log returns using threshold (0.02) ##

pos_ret_bin_2022_2023 <- rep (0, length (log_ret_bin_2022_2023))

## Assigning 1 if negative log returns are greater than threshold (0.02) and 0 otherwise ##
for (i in 1:length (log_ret_bin_2022_2023)) {
  if (log_ret_bin_2022_2023[i] > 0.02) {
    pos_ret_bin_2022_2023[i] <- 1
  }
}
...
week_summations_bin_2022_2023 <-calc_week_sum_bin(neg_ret_bin_2022_2023)

## Creating a data frame with negative quarters and their corresponding values ##
neg_quart_table_bin_2022_2023 <- data.frame(
  Weekly_bin = 1:104, # Weekly numbers
  Value = week_summations_bin_2022_2023 # Corresponding values
)
# Printing the resulting table

print (neg_quart_table_bin_2022_2023)
...
week_summations_1_bin_2022_2023 <- calc_week_sum_1_bin (pos_ret_bin_2022_2023)

## Creating a data frame with positive weeks and their corresponding values ##

pos_week_table_bin_2022_2023 <- data.frame(
  Week=1:104, #week numbers
  Value = week_summations_1_bin_2022_2023 # Corresponding values
)
# Print the resulting table

print (pos_week_table_bin_2022_2023)

## creating a cross tabulation table ##

cross_tabulation_bin_2022_2023 <-table (week_summations_bin_2022_2023,week_summations_1_bin_2022_2023)

## showing totals ##
extra_row_bin_2022_2023 <- matrix(rep(0,15),3,5)
rownames(extra_row_bin_2022_2023) <- c("5","6","7")
cross_tabulation_bin_2022_2023 <- rbind(cross_tabulation_bin_2022_2023,extra_row_bin_2022_2023)


extra_col_bin_2022_2023 <- matrix(rep(0,24),8,3)
colnames(extra_col_bin_2022_2023) <- c("5","6","7")
cross_tabulation_bin_2022_2023 <- cbind(cross_tabulation_bin_2022_2023,extra_col_bin_2022_2023)
#Adding row and column margins to the cross-tabulation table

cross_tab_with_totals_bin_2022_2023 <- addmargins (cross_tabulation_bin_2022_2023)

# Printing the table with subtotals and grand totals

print (cross_tab_with_totals_bin_2022_2023)

#

combined_data_bin_2022_2023 <- rbind(cross_tab_with_totals_bin_2022_2023[,9][-9], 
                                     cross_tab_with_totals_bin_2022_2023[9,][-9])

combined_data_rf_bin_2022_2023 <- combined_data_bin_2022_2023/104

rownames(combined_data_rf_bin_2022_2023) <- c("Negative","Positive")

# Create the bar plot with bars side by side of number of change in weekly returns

barplot(combined_data_bin_2022_2023, beside = TRUE, col = c("blue4", "lightblue"),
        names.arg = c(0:7),ylim = c(0,50),
        xlab = "Number of change in binancecoin weekly returns",
        ylab = "Frequencies of number of binancecoin weekly returns from 2020 to 2021")
...
chunks <- split_list(log_ret_bin_2022_2023, 7)
...
log_ret_bin_2022_2023 <- c(NA,log_ret_bin_2022_2023)
bin_2022_2023 <- cbind(bin_2022_2023,log_ret_bin_2022_2023)


ggplot(bin_2022_2023,aes(x =X2022.2023.date , y = log_ret_bin_2022_2023)) +
...
summary(log_ret_bin_2018_2019,na.rm  =T)
summary(log_ret_bin_2020_2021,na.rm  =T)
summary(log_ret_bin_2022_2023,na.rm  =T)
