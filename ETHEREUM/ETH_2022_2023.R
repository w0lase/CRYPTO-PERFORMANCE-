## Importing the ethereum data from 2022 to 2023 ##
...
eth_2022_2023 <- as.data.frame(eth_data_split[3],)

## Generating a vector for the ethereum prices ##

eth_2022_2023_prices <- eth_2022_2023$X2022.2023.price

## Calculating the log Returns ##

log_ret_eth_2022_2023 <- diff(log(eth_2022_2023_prices))

## Creating a vector to store the negative log returns using threshold (-0.02) ##

neg_ret_eth_2022_2023 <- rep (0, length (log_ret_eth_2022_2023))

## Assigning 1 if negative log returns are less than threshold (-0.02) and 0 otherwise ##

for (i in 1: length(log_ret_eth_2022_2023)) {
  if (log_ret_eth_2022_2023[i] < -0.02) {
    neg_ret_eth_2022_2023[i] <- 1
  }
}

## Creating a vector to store the positive log returns using threshold (0.02) ##

pos_ret_eth_2022_2023 <- rep (0, length (log_ret_eth_2022_2023))

## Assigning 1 if negative log returns are greater than threshold (0.02) and 0 otherwise ##
for (i in 1:length (log_ret_eth_2022_2023)) {
  if (log_ret_eth_2022_2023[i] > 0.02) {
    pos_ret_eth_2022_2023[i] <- 1
  }
}
...
week_summations_eth_2022_2023 <-calc_week_sum_eth(neg_ret_eth_2022_2023)

## Creating a data frame with negative quarters and their corresponding values ##
neg_quart_table_eth_2022_2023 <- data.frame(
  Weekly_eth = 1:104, # Weekly numbers
  Value = week_summations_eth_2022_2023 # Corresponding values
)
# Printing the resulting table

print (neg_quart_table_eth_2022_2023)
...
week_summations_1_eth_2022_2023 <- calc_week_sum_1_eth (pos_ret_eth_2022_2023)

## Creating a data frame with positive weeks and their corresponding values ##

pos_week_table_eth_2022_2023 <- data.frame(
  Week=1:104, #week numbers
  Value = week_summations_1_eth_2022_2023 # Corresponding values
)
# Print the resulting table

print (pos_week_table_eth_2022_2023)

## creating a cross tabulation table ##

cross_tabulation_eth_2022_2023 <-table (week_summations_eth_2022_2023,week_summations_1_eth_2022_2023)

## showing totals ##

extra_col_eth_2022_2023 <- matrix(rep(0,12),6,2)
colnames(extra_col_eth_2022_2023) <- c("6","7")
cross_tabulation_eth_2022_2023 <- cbind(cross_tabulation_eth_2022_2023,extra_col_eth_2022_2023)


extra_row_eth_2022_2023 <- matrix(rep(0,16),2,8)
rownames(extra_row_eth_2022_2023) <- c("6","7")
cross_tabulation_eth_2022_2023 <- rbind(cross_tabulation_eth_2022_2023,extra_row_eth_2022_2023)



#Adding row and column margins to the cross-tabulation table

cross_tab_with_totals_eth_2022_2023 <- addmargins (cross_tabulation_eth_2022_2023)

# Printing the table with subtotals and grand totals

print (cross_tab_with_totals_eth_2022_2023)

#

combined_data_eth_2022_2023 <- rbind(cross_tab_with_totals_eth_2022_2023[,9][-9], 
                                     cross_tab_with_totals_eth_2022_2023[9,][-9])

combined_data_rf_eth_2022_2023 <- combined_data_eth_2022_2023/104

rownames(combined_data_rf_eth_2022_2023) <- c("Negative","Positive")

# Create the bar plot with bars side by side of number of change in weekly returns

barplot(combined_data_eth_2022_2023, beside = TRUE, col = c("blue4", "lightblue"),
        names.arg = c(0:7),ylim = c(0,50),
        xlab = "Number of change in ethereum weekly returns",
        ylab = "Frequencies of number of ethereum weekly returns from 2020 to 2021")
...
chunks <- split_list(log_ret_eth_2022_2023, 7)
...
log_ret_eth_2022_2023 <- c(NA,log_ret_eth_2022_2023)
eth_2022_2023 <- cbind(eth_2022_2023,log_ret_eth_2022_2023)


ggplot(eth_2022_2023,aes(x =X2022.2023.date , y = log_ret_eth_2022_2023)) +
...
round(summary(log_ret_eth_2018_2019,na.rm  =T),4)
round(summary(log_ret_eth_2020_2021,na.rm  =T),4)
round(summary(log_ret_eth_2022_2023,na.rm  =T),4)
