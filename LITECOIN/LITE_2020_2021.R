## Importing the litecoin data from 2020 to 2021 ##
...
lite_2020_2021 <- as.data.frame(lite_data_split[2],)

## Generating a vector for the litecoin prices ##

lite_2020_2021_prices <- lite_2020_2021$X2020.2021.price

## Calculating the log Returns ##

log_ret_lite_2020_2021 <- diff(log(lite_2020_2021_prices))

## Creating a vector to store the negative log returns using threshold (-0.02) ##

neg_ret_lite_2020_2021 <- rep (0, length (log_ret_lite_2020_2021))

## Assigning 1 if negative log returns are less than threshold (-0.02) and 0 otherwise ##

for (i in 1: length(log_ret_lite_2020_2021)) {
  if (log_ret_lite_2020_2021[i] < -0.02) {
    neg_ret_lite_2020_2021[i] <- 1
  }
}

## Creating a vector to store the positive log returns using threshold (0.02) ##

pos_ret_lite_2020_2021 <- rep (0, length (log_ret_lite_2020_2021))

## Assigning 1 if negative log returns are greater than threshold (0.02) and 0 otherwise ##
for (i in 1:length (log_ret_lite_2020_2021)) {
  if (log_ret_lite_2020_2021[i] > 0.02) {
    pos_ret_lite_2020_2021[i] <- 1
  }
}
...
week_summations_lite_2020_2021 <-calc_week_sum_lite(neg_ret_lite_2020_2021)

## Creating a data frame with negative quarters and their corresponding values ##
neg_quart_table_lite_2020_2021 <- data.frame(
  Weekly_lite = 1:104, # Weekly numbers
  Value = week_summations_lite_2020_2021 # Corresponding values
)
# Printing the resulting table

print (neg_quart_table_lite_2020_2021)
...
week_summations_1_lite_2020_2021 <- calc_week_sum_1_lite (pos_ret_lite_2020_2021)

## Creating a data frame with positive weeks and their corresponding values ##

pos_week_table_lite_2020_2021 <- data.frame(
  Week=1:104, #week numbers
  Value = week_summations_1_lite_2020_2021 # Corresponding values
)
# Print the resulting table

print (pos_week_table_lite_2020_2021)

## creating a cross tabulation table ##

cross_tabulation_lite_2020_2021 <-table (week_summations_lite_2020_2021,week_summations_1_lite_2020_2021)

## showing totals ##

extra_col_lite_2020_2021 <- matrix(rep(0,10),5,2)
colnames(extra_col_lite_2020_2021) <- c("6","7")
cross_tabulation_lite_2020_2021 <- cbind(cross_tabulation_lite_2020_2021,extra_col_lite_2020_2021)


extra_row_lite_2020_2021 <- matrix(rep(0,24),3,8)
rownames(extra_row_lite_2020_2021) <- c("5","6","7")
cross_tabulation_lite_2020_2021 <- rbind(cross_tabulation_lite_2020_2021,extra_row_lite_2020_2021)


#Adding row and column margins to the cross-tabulation table

cross_tab_with_totals_lite_2020_2021 <- addmargins (cross_tabulation_lite_2020_2021)

# Printing the table with subtotals and grand totals

print (cross_tab_with_totals_lite_2020_2021)

# Generating row(negative) and column(positive) totals

combined_data_lite_2020_2021 <- rbind(cross_tab_with_totals_lite_2020_2021[,9][-9], cross_tab_with_totals_lite_2020_2021[9,][-9])

combined_data_rf_lite_2020_2021<- combined_data_lite_2020_2021/104

rownames(combined_data_rf_lite_2020_2021) <- c("Negative","Positive")


# Create the bar plot with bars side by side of number of change in weekly returns

barplot(combined_data_lite_2020_2021, beside = TRUE, col = c("blue4", "lightblue"),
        names.arg = c(0:7),
...
chunks <- split_list(log_ret_lite_2020_2021, 7)
...
log_ret_lite_2020_2021 <- c(NA,log_ret_lite_2020_2021)
lite_2020_2021 <- cbind(lite_2020_2021,log_ret_lite_2020_2021)


ggplot(lite_2020_2021,aes(x =X2020.2021.date , y = log_ret_lite_2020_2021)) +
